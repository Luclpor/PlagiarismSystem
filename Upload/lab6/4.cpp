...
int main() {
   ...
   while (!glfwWindowShouldClose(window)) {
       ...
       glm::vec3 lightColor(std::abs(std::sin(time)),
                            std::abs(std::cos(time)),
                            std::abs(std::sin(time * 2 + 0.5)));
   ...
   for (size_t i = 0; i < circles.size() - 1; i++) {
       for (size_t j = 0; j < circles[i].size() - 1; j++) {
           ...
           triangles.insert(triangles.end(), {
                   ...
                   circles[i + 1][j].first, circles[i + 1][j].second, z + zStep, n.x, n.y, n.z, figureR, figureG,
                   figureB,
           });
           ...
           triangles.insert(triangles.end(), {
                   circles[i][j + 1].first, circles[i][j + 1].second, z, n.x, n.y, n.z, figureR, figureG, figureB,
                   circles[i + 1][j + 1].first, circles[i + 1][j + 1].second, z + zStep, n.x, n.y, n.z, figureR,
                   figureG, figureB,
                   circles[i + 1][j].first, circles[i + 1][j].second, z + zStep, n.x, n.y, n.z, figureR, figureG,
                   figureB,
           });
